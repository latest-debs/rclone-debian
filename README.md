![rclone for Debian](.github/readme-header.png)

# rclone for Debian

[rclone/rclone](https://github.com/rclone/rclone) — Syncs files and directories to and from cloud storage providers (S3, Drive, Dropbox, etc.) via the command line. —
...
Or download a `.deb` from the [Releases](https://github.com/latest-debs/rclone-debian/releases) page:

```sh
sudo dpkg -i rclone_*.deb
```
...
- amd64, arm64, armhf, i386, ppc64el, riscv64, s390x — whichever
  architectures rclone/rclone actually publishes a Linux binary for
...
For issues with rclone itself, see
